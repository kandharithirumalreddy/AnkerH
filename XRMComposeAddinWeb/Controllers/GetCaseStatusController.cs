...
{
    public class GetCaseStatusController : ApiController
    {
        // GET api/<controller>
        ...
        List<string> statuses =new List<string>();
          ...
          var lstatus = await graphClient.Sites[siteId].Lists[caselistId].Columns["17216dcb-35ca-4675-8838-818ac63fdc30"].Request().GetAsync();
          // cases = lcases.Choice.Choices;

          foreach (var lcase in lstatus.Choice.Choices)
          {
            statuses.Add(lcase);
          }
                ...
                return ResponseMessage(new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent(JsonConvert.SerializeObject(statuses, Formatting.Indented), Encoding.UTF8, "application/json") });
            }
            catch(Exception ex)
            {
                var except = ex;
