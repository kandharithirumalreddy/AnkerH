...
namespace XRMComposeAddinWeb.Controllers
{
  public class GetUserDefaultConfigController : ApiController
  {
    // GET api/<controller>
    [HttpGet]
    public async Task<IHttpActionResult> Get([FromBody]string useremail)
    {
    ...
    private async Task<IHttpActionResult> GetUserDefaultConfig(string useremail)
    {
      ...
      if (bootstrapContext != null)
      {
        ...
        // string filterString = " contains(UsersMail, '" + useremail + "')";
        string filterString = "UsersMail eq " + useremail;
        List<QueryOption> options = new List<QueryOption>()
          {
          new QueryOption("filter", filterString),
          };

        var lcases = await graphClient.Sites[siteId].Lists[defaultCongigId].Items.Request(options).Top(1).GetAsync();
        ...
        //        UserMail = lcase.Fields.AdditionalData["UserMail"].ToString()
        //    });
        //}
        if (lcases != null && lcases.Count > 0)
        {
          var lcase = lcases[0];
          userinfo.Add(new GetUserDefaultConfigInfo()
          {
